#' S3 Methods for margot.core Objects
#'
#' @description
#' This file contains S3 generic methods and class definitions for the
#' margot.core package, providing the fundamental object system for shadows,
#' scenarios, and panel data used across the margotsphere.
#'
#' @details
...
print.margot_shadow <- function(x, ...) {
  ...
  for (name in param_names) {
    ...
    if (is.function(value)) {
      ...
      cat("  ", name, ": ", paste0(utils::head(value, 3), collapse = ", "), ", ...\n", sep = "")
    } else if (is.numeric(value) && length(value) > 5) {
      cat("  ", name, ": ", paste0(utils::head(value, 5), collapse = ", "), ", ...\n", sep = "")
    } else {
...
print.margot_scenario <- function(x, ...) {
  ...
  if (!is.null(x$references) && length(x$references) > 0) {
    cat("References:", paste(x$references, collapse = "; "), "\n")
  }
  
  cat("Number of shadows:", length(x$shadows), "\n")
  
  if (length(x$shadows) > 0) {
    shadow_types <- vapply(x$shadows, function(s) s$type, character(1))
    cat("Shadow types:", paste(shadow_types, collapse = ", "), "\n")
  }
...
print.shadow_list <- function(x, ...) {
  n <- length(x)
  cat("<shadow_list> with", n, if (n == 1) "shadow" else "shadows", "\n")
...
as.data.frame.margot_scenario <- function(x, ...) {
  ...
  shadow_dfs <- lapply(seq_along(x$shadows), function(i) {
    ...
    # add key parameters based on shadow type
    if (shadow$type == "measurement_error") {
      df$variables <- paste(shadow$params$variables, collapse = ";")
      df$error_type <- shadow$params$error_type
      if (!is.null(shadow$params$sigma)) df$sigma <- shadow$params$sigma
    } else if (shadow$type == "missing_data") {
      df$mechanism <- shadow$params$mechanism
      df$prob <- shadow$params$prob
    }
    
    df
  })
  
  # bind rows with fill = TRUE to handle different columns
  result <- do.call(function(...) {
    dfs <- list(...)
    all_cols <- unique(unlist(lapply(dfs, names)))
    
    # ensure all dfs have all columns
    dfs <- lapply(dfs, function(df) {
      missing_cols <- setdiff(all_cols, names(df))
      for (col in missing_cols) {
        df[[col]] <- NA
      }
      df[all_cols]  # return with consistent column order
    })
